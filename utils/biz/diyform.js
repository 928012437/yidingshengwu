    ...
    DiyFormHandler: function(a, t) {
        ...
        s[f][n.name] = t.detail.value) : s[f] = t.detail.value; else if ("picker" == d) {
            for (var y in s) if (y == f) {
                for (var c in m) if (m[c].diy_type == f) {
                    s[f] = [ t.detail.value, m[c].tp_text[t.detail.value] ];
                    break;
                }
                break;
            }
            ...
            for (var y in s) if (y == f) {
                ...
                for (var c in s[f].images) s[f].images[c].filename != n.filename && p.images.push(s[f].images[c]);
                p.count = p.images.length, s[f] = p;
                break;
            }
        } else if ("image-preview" == d) for (var y in s) if (y == f) {
            var v = [];
            for (var c in s[f].images) v.push(s[f].images[c].url);
            wx.previewImage({
                current: v[n.index],
                urls: v
            });
